...
# return_ddpg_her = json.load(open(os.path.join(path, 'exp_multi_goal', 'ddpg_pybullet_gym', 'Push_HER', 'return_statistic.json')))
return_ddpg_pher = json.load(open(os.path.join(path, 'exp_multi_goal', 'ddpg_pybullet_gym', 'Push_PHER', 'return_statistic.json')))
# return_sac_her = json.load(open(os.path.join(path, 'exp_multi_goal', 'sac_pybullet_gym', 'Reach_HER', 'return_statistic.json')))
return_sac_pher = json.load(open(os.path.join(path, 'exp_multi_goal', 'sac_pybullet_gym', 'Push_PHER', 'return_statistic.json')))

plot.smoothed_plot_mean_deviation(os.path.join(path, 'src', 'returns_pybullet_kuka_push.png'),
                                  [return_ddpg_pher, return_sac_pher],
                                  legend=['DDPG PHER', 'SAC PHER'],
                                  legend_loc='lower right',
                                  title='Pybullet Kuka Push (4 seeds)',
                                  window=20,
